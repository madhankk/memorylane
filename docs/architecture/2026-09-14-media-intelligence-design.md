...
- **Faces:** **Implemented in Phase 4 as YuNet + SFace (OpenCV Zoo, Apache-2.0, 128-d).** InsightFace `buffalo_l` (ArcFace 512-d) is stronger but research-only licensed; it remains a possible opt-in provider since the face model id is part of every vector's space.
- **Clustering:** **Implemented as Chinese whispers over a cosine kNN graph** (numpy, block-wise similarities, linear memory) instead of HDBSCAN — no scikit-learn/scipy dependency and no O(N²) distance matrix.
...
1. **Detect** (`faces` analyzer, sidecar): per image → faces rows. `quality = det_score × min(1, longest edge / 0.05)`; faces below 0.5 are stored but never seed a person, so a crowd of 40 blurry heads doesn't spawn 40 "persons". (Phase 4 as built.)
2. **Assign incrementally** (in-process, `PersonService.assignNewFaces`): for each new quality face, kNN via the VectorIndex restricted to faces with a `person_id`. During an initial backfill this step is skipped (nothing is labelled yet) and step 3 does the bulk work; incremental assignment is the steady-state path for new photos. If the best match's cosine ≥ `faceAssignThreshold` (default 0.45 for SFace, from measured same/different-person separation; setting) and that person isn't in the face's rejections, assign `assigned_by='auto'`. This keeps known people up to date as new photos arrive without re-clustering the world.
...
| Face models (Phase 4) | ✅ | ✅ | ✅ | **Implemented as YuNet (detector) + SFace (128-d recogniser) from OpenCV Zoo, both Apache-2.0**, loaded directly through `onnxruntime` — no `insightface` pip package (C++ toolchain on Windows) and no non-commercial license (InsightFace `buffalo_l` is research-only, incompatible with distributing an MIT app). Measured 98 ms/image on M2 Max CPU; same-person cosine 0.64–0.89, different 0.11–0.19. |
